...
export default function EventScreen({ view, shipView, onChoose }: EventScreenProps) {
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center px-4">
      <div className="flex w-full max-w-md flex-col gap-5 border-2 border-fd-steel/40 bg-fd-void px-6 py-7">
        <span className="font-label text-xl uppercase text-fd-orange sm:text-3xl">
          {view.title}
        </span>
          ...
          <div className="flex w-full max-w-md flex-col gap-3">
            {view.choices.map((choice, index) => (
              <button
                ...
                onClick={() =>
                  choice.requiresModuleTarget ? setTargeting(index) : onChoose(index)
                }
