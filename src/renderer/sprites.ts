...
const R = (g: Ctx, x: number, y: number, w: number, h: number, c: string): void => {
  g.fillStyle = c;
...
const outline = (g: Ctx, w: number, h: number, col: string): void => {
...
function makeCanvas(w: number, h: number): { canvas: HTMLCanvasElement; g: Ctx } {
...
const make = (w: number, h: number, fn: (g: Ctx) => void, oc?: string): HTMLCanvasElement => {
