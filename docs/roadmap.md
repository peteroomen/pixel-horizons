...
- [x] **CB.0 Feel prototype (throwaway)** — _shipped (PR #32)._ `/core-breaker-spike` harness; the pivot was confirmed fun and made real.
- [x] **CB.1 Deterministic physics core (sim only)** — _shipped (PR #33)._ `surface/core-breaker.ts` seeded fixed-timestep circle-vs-peg sim + tests. (Mining v2, PR #35, evolved it: pod-at-top, drops magnet up, no catch.)
- [x] **CB.2 Field generation** — _shipped (PR #33; v2 layout in PR #35)._ `surface/field-gen.ts` deterministic peg field (LS/PT dual layout) + tests.
- [x] **CB.3 Module → ball projection** — _shipped (PR #34)._ `surface/ball-projection.ts` — `projectMiningRoster(modules)` → the firing roster; Mk tier buffs both faces.
- [x] **CB.4 Core Breaker renderer + playable** — _shipped (PR #34 route; PR #38 visual pass + portrait + React HUD + orbit DROP→mining wiring)._ `renderer/core-breaker-renderer.ts` + `renderer/core-breaker/*` (pixel-art sprites, crust bg, React HUD). Live in-game (`enterMining`, portrait stage) **and** `/core-breaker` — one `startCoreBreaker` session, two entry paths. `?mode=mining` dev knob.
- [ ] **CB.5 Ball glyph grammar + module UI** — _visual sprites shipped (PR #38); glyph grammar + dual-face `<ModuleCard>` still TODO._ Trajectory glyph per ball (straight/arc/curve) + count badge; the dual-face `<ModuleCard>` showing combat + surface faces (§6.4 UI law) across Workbench/DeckViewer/shop/bag preview at 375px. Coordinate with 4.8/6.7 station-UX.
...
- [ ] **6.1 Visual identity pass** — Deep-Fold integration (planets, backgrounds, ship bases), Resurrect 64 palette lock, modular ship sprite with visible module slots. *(Combat-screen world-art shipped — `docs/work/2026-06-15-world-art-direction.md` / `docs/design/foundry-world-art-direction.dc.html`.)* **Planets are now RUNTIME-generated** via ported Deep-Fold GLSL through PixiJS v8 — not a pre-exported sprite pool (ADR 010 supersedes ADR 002 on this; feasibility spike done: `docs/work/2026-06-16-planet-shader-spike.md`). First slice planned: `docs/work/2026-06-16-6.1-runtime-planets.md` (generation + deterministic run integration + `palette.ts` R64 ramps; surface-recolor/backdrops/more-types are follow-on slices).
  ...
  - **Shield bubble design pass (future):** the combat shield bubble is a single fixed ellipse that doesn't fit the different hull silhouettes. A per-hull `HULL_SHIELD` ellipse attempt was tried and **shelved** (still didn't read right) — the shield visual wants a proper design pass, not just per-hull radii: shape/scale to the silhouette, layered/multi-ring states (FTL-style layers already exist in sim), hit-flicker, and an up/down feel. *(Shelved 2026-06-19; revisit as a dedicated shields visual pass.)*
