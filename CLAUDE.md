...
- **Current phase:** Phase 4/5/6. THE ARSENAL mega-slice (4.6 card keywords + 4.4 events + boss organs + the 2026-06-15 playtest fixes) is **merged** (PR #13). **WORLD ART DIRECTION** (Slice 6.1, combat-screen scope) is landing now (PR #14, `feature/world-art`, plan: `docs/work/2026-06-15-world-art-direction.md`) — the combat renderer uses the real pixel-art from the Claude Design handoff (`docs/design/foundry-world-art-direction.dc.html`). Next candidate: **Slice 4.7** (Powers & status effects — GDD §5.10), with **3.4** (clone death + surface enemies) and **4.8** (Station UX & workbench access) also queued.
- **Last session:** 2026-06-15 — PR #13 playtest fixes on the arsenal (see `docs/work/2026-06-15-arsenal-playtest-fixes.md`): (1) **boss out of lanes** — `boss` flag on `EnemyDef`, set on `enemy-gatemaw`, filtered out of `DEFAULT_ENEMY_POOL` in `travel.ts` (the boss was rolling into random lane encounters repeatedly — feedback #1/#5; the gate fight was already single-encounter); (2) **Spore Cluster clearable** — `card-spore-cluster` gains `jettison`+`exhaust`, and `jettisonCard` now routes exhaust cards to the exhaust pile so the clog clears for good (feedback #4); (3) **buy-block reason** — `ShopOfferView.blockReason` (`need-slot`/`need-scrap`) surfaced in `StationScreen` so a blocked purchase says why (feedback #3); (4) **mobile tap hardening** — `touch-none` scoped to the canvas host (off `<main>`) + `touch-manipulation` on `FoundryButton` (the engineer-repair "wild clicking", feedback #3 — physical-phone only, not desktop-reproducible); (5) **organ targeting legibility** — `EnemyPlate` organ/core rows get a "TAP A TARGET TO FOCUS FIRE" hint, larger text, bordered rows, and a `◎ TARGET` focus badge (feedback #6). 421 tests green; browser-verified 375 px (Gatemaw organ focus-fire moves the badge; Spore Cluster ⤓ jettison clears the card). **Earlier 2026-06-15 (THE ARSENAL itself, 419 tests):** per-card malfunction repair, Retain/Jettison/Discard/Cleave keyword engine, RunState v3 + modifiers, deterministic event system, boss organs; ADRs 006/007.
- **All merged to main:** Scaffold, Slices 1.2–1.3 (PRs #1–2), 2.1–2.5 (PRs #3–7), 3.1 (PR #8), FOUNDRY Combat UI (PR #9), 3.2 (PR #10), THE RUN mega-slice 3.3+4.1+4.5-lite (PR #11), THE FORGE mega-slice 4.2+4.3+5.2 (PR #12), THE ARSENAL mega-slice 4.6+4.4+organs (PR #13).
- **Open PRs:** #14 WORLD ART DIRECTION (`feature/world-art`) — combat-screen pixel-art renderer.
- **Playtest follow-ups (PR #13, in the roadmap):** #2 STS-style Powers/status system + Exhaust-on-buff + targeted-debuff skills + keyword tooltips → **4.7** (GDD §5.10 direction written); #3 full module-management UX + #7 workbench access from stations → **4.8**; "too easy" combat + first-try boss kill → 5.5 / 5.2 boss tuning. Phone-verify still pending for the buy-block label and the engineer-repair tap (the touch bug never reproduced on desktop).
