...
- **Current phase:** Phase 6 identity work (3.4, 4.7, 4.8, 6.6, combat + surface world-art all merged). **RUNTIME PLANETS (6.1 slice 1)** is **on branch `feature/runtime-planets`** (`docs/work/2026-06-16-6.1-runtime-planets.md`, ADR 010) — planets are now **generated at runtime** via a ported Deep-Fold GLSL shader through PixiJS v8, deterministic from the run seed and palette-locked to Resurrect 64 (`renderer/palette.ts`). A planet node now arrives into a new **orbit screen** (`'orbit'` phase + `orbit-mode`/`orbit-renderer` — a baked `RenderTexture` planet + starfield + DROP) → surface. Sim descriptor `planetForNode(seed, nodeId)` is **derived, not stored** (no `RUN_STATE_VERSION` bump). `?mode=orbit` dev knob added; `/planet-spike` is a throwaway dev harness. 512 tests green; lint/type-check clean; browser-verified (orbit → DROP → surface). **Surface-tileset recolor = slice 2 (with 5.3); more planet types / atmosphere / space-bg = later 6.1 slices.** Next: 6.1 slice 2 (surface recolor), 4.9 deckbuilding audit, 5.5 balance.
- **Last session:** 2026-06-18 — **RUNTIME PLANETS 6.1 slice 1** (above): ADR 010 (supersedes ADR 002 on planets — runtime generation, not a pre-exported pool, validated by a spike); `renderer/palette.ts` (R64 + seed ramps, 9 tests), `data/planets.ts` + `sim/planet.ts` (descriptor, 4 tests), `renderer/planet.ts` (RT bake, loose `vec3[6]` ramp uniforms, hard-`discard` halo fix), orbit-screen wiring (`main.ts`/`GameCanvas`/`page.tsx`), `docs/credits.md`. Two PixiJS-v8 gotchas pinned: `Shader.from` resource uniforms are **loose** (a UBO block → silent `INVALID_OPERATION`, skipped draw); large seeds overflow float32 in-shader → moved variety to a bounded `vec2` noise offset.
- **All merged to main:** Scaffold, Slices 1.2–1.3 (PRs #1–2), 2.1–2.5 (PRs #3–7), 3.1 (PR #8), FOUNDRY Combat UI (PR #9), 3.2 (PR #10), THE RUN (PR #11), THE FORGE (PR #12), THE ARSENAL (PR #13), WORLD ART DIRECTION combat renderer (PR #14), POWERS & STATUS 4.7 (PR #15), STATION UX 4.8 (PR #16), CLONE DEATH 3.4 (PR #17), ECONOMY & SHIP-SLOTS (PR #18), COMBAT JUICE 6.6 (PR #19), SURFACE OVERWORLD ART (PR #20). Plus docs direct to main: ADR 010 + runtime-planet plans/handoff.
- **Open PRs:** RUNTIME PLANETS 6.1 slice 1 (`feature/runtime-planets`) — PR pending.
