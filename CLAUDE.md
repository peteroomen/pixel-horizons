...
- **Current phase:** Phase 2 in progress. Phase 1 complete; Slice 2.1 (combat engine, sim only) complete. Next: Slice 2.2 (combat playable — React card hand + Pixi viewport + HUD).
- **Last session:** 2026-06-11 — Slice 2.1: combat engine in `src/game/sim/combat.ts` — plain-JSON `CombatState` (serializable mid-fight, ADR-003 RNG-by-value), `createCombat`/`playCard`/`endTurn`/`currentIntent`, interpreter for 13 of 15 `CardEffect` kinds (loud throw-stubs: `strip-armor`→2.5, `repair-all-modules`→2.3), hit-based shield layers with per-layer recharge, piercing, temp layers, Lamprey in `src/game/data/enemies.ts` (telegraphed intent cycle), `BASELINE_AP`/`HAND_SIZE` in `data/constants.ts`, `createRunState` now seeds hull starting modules (no version bump). 101 tests green, no UI. See `docs/work/2026-06-10-combat-engine.md` and the Slice 2.2 handoff `docs/work/2026-06-11-handoff-slice-2.2.md`.
- **All merged to main:** Scaffold, Slice 1.2 (PR #1), Slice 1.3 (PR #2).
- **Open PRs:** Slice 2.1 — `feature/combat-engine`.
- **Deferred:** Applying combat results back to RunState (rng.combat/hull HP/scrap — first wiring consumer, 2.2/2.4); hull innate abilities (2.3); retain-with-choice API if 2.2 wants it; Mk II deck generation + tier tracking in RunState (Workbench, 4.2); wiring seed-url/`SaveStore` into the page (lands with the first UI consumer); physical-phone check of production URL; sprite-motion smoothness by eye; `motion` installed but unused until the card-feel slice.
