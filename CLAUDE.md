...
<<<<<<< feature/surface-overworld-art
- **Current phase:** Phase 6 identity work, pulled forward (3.4, 4.7, 4.8 all merged). **SURFACE OVERWORLD ART** is **in review on branch `feature/surface-overworld-art`** (PR open, `docs/work/2026-06-16-surface-overworld-art.md`) — the surface twin of combat's World Art Direction pass (#14): the surface renderer now uses the design's Resurrect-64 ramped pixel-art (`Foundry - Overworld Art Direction.dc.html`) instead of flat-colour `Graphics`. Next candidates: FOUNDRY surface-HUD restyle (§10, DOM), live LUT biome pipeline (§1 → 5.3), **4.9** deckbuilding acquisition audit, 5.5 balance pass.
- **Last session:** 2026-06-16 — **SURFACE OVERWORLD ART** (`feature/surface-overworld-art`, `docs/work/2026-06-16-surface-overworld-art.md`): ported the overworld art-direction doc into the renderer. New **`renderer/sprite-primitives.ts`** (shared `R`/`mix`/`rng`/`blob`/`dither`/`outline`/`make`, extracted from `sprites.ts` — combat art untouched) + **`renderer/surface-sprites.ts`** (terrain ×10 with autotile-lite surface/fill crust, clone poses ×5, pod camp/launch, fauna ×3, bramble/crumble tiles, corpse, items ×3 tinted by source, mining-shatter + landing-dust FX, cave-mouth parallax backdrop + drifting spore motes). `surface-renderer.ts` rewired to nearest-neighbour textures/sprites: derives clone pose + pod state + fauna facing from sim, adds an FX ticker + ambient descent gloom, recoloured slash/shield/beacon to FOUNDRY accents. **Two palettes, one law** held — pure accents only on signals; the core-crystal glint is the one saturated terrain pixel. No sim changes; 490 tests green; lint/type-check/`pnpm build` clean. **Visual/feel/375px = pending human step.** Deferred to follow-ups: surface-HUD restyle (§10), live LUT + Volcanic/Ice (§1→5.3), 47-rule autotile atlas, heat-shimmer + multi-rate parallax scroll, shield pop-ripple frame, module-driven clone silhouette overlay.
- **All merged to main:** Scaffold, Slices 1.2–1.3 (PRs #1–2), 2.1–2.5 (PRs #3–7), 3.1 (PR #8), FOUNDRY Combat UI (PR #9), 3.2 (PR #10), THE RUN mega-slice 3.3+4.1+4.5-lite (PR #11), THE FORGE mega-slice 4.2+4.3+5.2 (PR #12), THE ARSENAL mega-slice 4.6+4.4+organs (PR #13), WORLD ART DIRECTION combat renderer (PR #14), POWERS & STATUS Slice 4.7 (PR #15), STATION UX & WORKBENCH ACCESS Slice 4.8 (PR #16), CLONE DEATH + SURFACE ENEMIES + ROCKY HAZARDS Slice 3.4 (PR #17).
- **Open PRs:** SURFACE OVERWORLD ART (`feature/surface-overworld-art`) — surface renderer pixel-art pass.
=======
- **Current phase:** Phase 3/4/5/6. **Slice 6.6 Combat juice (first cut)** is **built on branch `feature/combat-juice`** (`docs/work/2026-06-16-slice-6.6-combat-juice.md`) — pulled forward per the 2026-06-15 playtest ("not tactile/engaging"), scoped **combat-only** and **renderer/UI-only** (no sim, no `CombatView` change) so it runs **in parallel with the economy/shield-slot branch** (`docs/work/2026-06-16-handoff-economy-shield-slot.md`) with zero file overlap. All fx are **derived from consecutive `CombatView` snapshots** (combat pushes one per event, never per frame): new pure `combat-fx-core.ts` (`hpDrop`/`shakeAmplitude`, node-tested) + React `combat-fx.tsx` (`useDamageFloaters`/`DamageFloaters`/`HitFlash`/`PopPip`) give floating damage numbers + brightness flash + micro-shake on ship/core/**each organ** (co-located per-element hooks; `OrganRow` extracted so each can own a hook), popping AP/shield pips, **Motion** card hover-lift + tap-punch + `AnimatePresence` deal-in/play-out (id+occurrence-ordinal keys so only the played card animates out), and a `space-renderer.ts` **viewport shake** (HP-delta driven, damped sinusoid not RNG, backdrop overscanned by `ceil(MAX_SHAKE/PX)` tiles so the void never shows, `prefers-reduced-motion`-gated via `matchMedia`). `EnemyPartView` re-exported from `main.ts`. **497 tests green (7 new); lint/type-check/`pnpm build` clean; browser-verified desktop + 375px vs Gatemaw (enemy core floater `−3` ⇄ core 70→67, no console errors).** Human feel/phone pass + reduced-motion visual check pending; not yet committed/pushed. Before it: **Slice 3.4 Clone death + surface enemies + hazards** is **built on branch `claude/tender-allen-kgp133`** (`docs/work/2026-06-15-slice-3.4-clone-death.md`, ADR 009) — clone HP/i-frames/hit-stun/knockback + death → single-instance **corpse** drop + corpse-run recovery + free-first/Scrap **re-print** (economy gated in `main.ts`, sim stays economy-free per the 3.2 invariant); **Shield Bubble turned on** (absorbs 1 hit, cooldown recharge, flings a Ceiling Dropper on absorb); 3 enemies (Bloom Hopper leap, Scrap Grubber passive→aggro/cornered, Ceiling Dropper drop+landing-stun) placed by tile-tokens `H/G/C`; 3 Rocky hazards (Spike Bramble `^` non-solid, Crumbling Sandstone `~` state machine, Sandstorm Vent `V` push via a new `updateClone` `externalVx`); enemy drops → backpack with floor-bounce world items + magnetism. New sim modules `surface/enemies.ts`/`hazards.ts`/`drops.ts`; matrix HP/melee/regen are now structured `PlanetItemEffect`s. Renderer adds enemies/items/corpse + beacon/shield ring/blink/red-flash + shake/death fade; HUD HP pips + shield + Cloning Bay overlay. **No `RUN_STATE_VERSION` bump** (clone/re-print state is ephemeral per-drop). 490 tests green; lint/type-check/`pnpm build` clean; dev serves `?mode=surface` 200. **375px/phone + feel pass pending (human step); push to origin currently 403s (env token).** Before it: **Slice 4.8 Station UX & workbench access** is **built on branch `claude/sharp-wright-iv37l2`** (`docs/work/2026-06-15-slice-4.8-station-ux.md`) — Workbench now reachable from Merchant/Engineer nodes (WORKBENCH button beside Leave), module/cargo/offer rows expand to preview their generated cards (new `describeModuleCards` reuses combat card text + keyword chips via the 4.7 `InfoChip`), per-slot **used/limit** picture in the workbench + a compact slot summary in the shop, and the buy-block reason is now the **full picture** (`OfferBlock`: slot name + X/Y full, or `Need N scrap · have M`) instead of the old `need-slot`/`need-scrap` labels. New sim helper `slotUsage` keeps the rule in the sim; all legibility is view/UI. 441 tests green; lint/type-check/`pnpm build` clean; **375px browser check still pending (human step).** Before it: **Slice 4.7 Powers & status** is **merged** (PR #15) — STS-style stacking statuses + tooltips, buff cards Exhaust, Tracer Lock a targeted Mark; 375px tooltip-portal fix. Next candidates: **3.4** (clone death + surface enemies) and **4.9** (deckbuilding acquisition audit).
