...
- **CB.0 FEEL PROTOTYPE merged (PR #32, squashed to main) + DIRECTION CONFIRMED.** The throwaway `/core-breaker-spike` harness (drag-aim · release-fire, hand-rolled circle-vs-peg on a 240 Hz fixed-timestep accumulator, 3 ball roles pierce/bouncy/homing, per-peg hit cooldown, live knobs) is in main; the user has **confirmed the Core Breaker pivot as the direction** — we're making it real. Pure physics in `src/app/core-breaker-spike/core-breaker-spike.ts`, Pixi/input/HUD in `page.tsx` (both throwaway, delete at CB.4). Start numbers for CB.1: G≈900, e≈0.72, ballR=6, pegR=7, ~46 pegs, 9 shots. **Next: CB.1 — deterministic seeded physics core** `src/game/surface/core-breaker.ts` (sim-only, fixed-timestep, Vitest determinism). **Cold-start handoff written: `docs/work/2026-06-20-handoff-cb1-core-breaker-core.md`** (incl. the API shape, the determinism rules, the peg-kind set, and a ⚠️ flag that `physics.ts` retirement must wait for CB.7 — it's still wired to the live platformer).
...
- **Last session:** 2026-06-20 — **CB.0 spike merged (PR #32) + CB.1 handoff written.** Merged the pivot docs (PR #30) and the throwaway `/core-breaker-spike` feel prototype (PR #32, both squashed to main). User confirmed the Core Breaker pivot as the direction; wrote the cold-start handoff for **CB.1 deterministic physics core** (`docs/work/2026-06-20-handoff-cb1-core-breaker-core.md`) — port the spike solver into a pure, seeded, fixed-timestep `src/game/surface/core-breaker.ts` with Vitest determinism; field-gen stays CB.2, `physics.ts` retirement stays CB.7. Next: build CB.1.
...
- **Open PRs:** [#24 Hull variety sprites](https://github.com/peteroomen/pixel-horizons/pull/24) (`feature/ship-sprites`) — browser verify pending. [#25 Atmosphere/sky recolor](https://github.com/peteroomen/pixel-horizons/pull/25) (`feature/atmosphere-sky-recolor`) — browser verify pending.
