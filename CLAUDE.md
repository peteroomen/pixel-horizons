...
- **Current phase:** Phase 4/5. THE ARSENAL mega-slice (4.6 card keywords + 4.4 events + boss organs) is on branch `feature/the-arsenal` (plan: `docs/work/mega-slice-4-plan.md`), ready for PR. Next candidate: **Slice 3.4** (clone death + hazards + surface enemies) — split out of THE ARSENAL for a GDD §6.7 surface-threat design pass; see handoff.
- **Last session:** 2026-06-15 — THE ARSENAL (8 layer-ordered commits, 419 tests green). **Part A (4.6):** per-card malfunction repair (the multi-turn AP tax fix — `CombatCard.malfunctioning`, no more `CombatState.malfunctioning`); Retain/Jettison/Discard/Cleave keyword engine + signature keyword per module slot (engine cards now dual-mode travel+Draw with Jettison — travel-card-at-boss fix); keyword chips + cyan ⤓ Jettison button in `CombatHand`. **Part B (4.4):** `RunState` v3 + attach-to-module modifiers (`data/modifiers.ts`, applied as per-instance `apCostDelta`/`bonusEffects` in `deck.ts`); deterministic event system (`data/events.ts`, `sim/events.ts` `pickEvent`/`applyEventChoice`, `event-view.ts`, `EventScreen.tsx`); `'event'` node type + phase. **Part C (§5.4):** boss-only targetable organs (`EnemyPart`, Gatemaw Spore-Sac + Armor-Node), `partHp` + `selectTarget` focus, Cleave routing, organ bars in `EnemyPlate`. Browser-verified desktop + 375 px (keyword chips, Jettison +1 AP, event resolve-to-map, Gatemaw organ focus-fire). ADRs 006/007 added.
...
- **Open PRs:** THE ARSENAL (`feature/the-arsenal`) — opening now.
