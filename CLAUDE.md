...
- **Last session:** 2026-06-20 — **CB.1 deterministic physics core built** (`docs/work/2026-06-20-cb1-core-breaker-core.md`). Merged CB.0 (PR #32); ported the spike solver into the pure sim `src/game/surface/core-breaker.ts` + 15 Vitest tests (544 total green). Two bugs caught + fixed: pierce drag was applied every overlapping sub-step (ball lingered → multi-hit; now gated once per peg by the hit cooldown); the no-tunnel contrast test was sample-position-sensitive (moved the peg into the 60 Hz gap). Additive — `physics.ts` untouched. Next: CB.2 field-gen.
