...
- **✅ CB.5 + CB.6 + CB.7 COMPLETE (2026-06-22, branch `claude/handoff-review-cb-tasks-vv5var`).** (`docs/work/2026-06-22-cb5-cb6-cb7.md`) Ball glyph grammar: `BALL_GLYPH` map in `data/core-breaker.ts`; `ballFace` on both view types in `ship-view.ts`; `BallGlyphIcon.tsx` with inline SVG glyphs; Workbench `ModuleRow` shows compact surface face chip; HUD queue shows glyph + count badges. Bloom soft-fail: `calcBloomDrain()` pure function in `mining-run.ts`; renderer tracks `bloomShotsActive`; `bloomThreat` in `CoreBreakerHudState`; "BLOOM ▲N" chip in HUD header; 5 unit tests. Platformer cleanup: 27 retired files deleted; `main.ts`/`page.tsx`/`GameCanvas.tsx` stripped of all surface path code; `?mode=surface` → `?mode=mining` alias. **387 tests green**, type-check/lint clean. **Next: 4.9 deckbuilding audit or 5.5 balance pass (re-judge with human play of the current mining loop).**
- **Last session:** 2026-06-22 — **CORE BREAKER: portrait + visual pass + full-screen + unified session + React HUD**, merged to main via PR #38 (see above). 562 tests green.
