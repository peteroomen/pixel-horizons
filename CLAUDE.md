...
- **Current phase:** Phase 6 / Core Breaker pivot. **MINING RUN V2 INTEGRATION built (branch `feature/mining-v2-slice-a`, PR open)** (`docs/work/2026-06-21-mining-v2-slice-a.md`): v2 ball types (standard/heavy/split/drill/ghost), v2 peg kinds (mineral/ore/hard/crystal/rock/bloom), staggered 8×9 grid field-gen (no bounce budget, no reachability filter), ghost passes through bloom, drill bores non-rock, split forks once, heavy 2× damage, pod-catch for re-aim, mineral drops as physics objects, `'mining'` GamePhase, `enterMining()` in main.ts, `dropToSurface()` → mining (not platformer), haul banks to run.resources on complete. `projectMiningRoster` replaces `projectSurfaceBag` (engine→drill, shield→ghost, HEAVY→heavy, SPLIT→split). **562 tests green**, type-check/lint/build clean. **Next: CB.5 ball glyph grammar + dual-face `<ModuleCard>`, CB.6 Bloom soft-fail, CB.7 platformer cleanup.**
- **Last session:** 2026-06-22 — **MINING RUN V2 INTEGRATION** (see above). 562 tests green. Branch `feature/mining-v2-slice-a`.
