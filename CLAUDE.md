...
- **CB.0 FEEL PROTOTYPE built (branch `claude/brave-hamilton-yds2fo`, pushed — PR not yet opened)** — a **throwaway** `/core-breaker-spike` harness (cloned from `/planet-spike`): drag-to-aim · release-to-fire, hand-rolled circle-vs-peg physics on a **240 Hz fixed-timestep accumulator** (anti-tunneling; non-deterministic — `Math.random()` field layout, _this spike only_), 3 ball roles (pierce/bouncy/homing), per-peg hit cooldown, on-screen ball buttons + live knob tuning (Q/A grav, W/S rest, E/D pwr, R reset). Pure physics in `core-breaker-spike.ts`, Pixi/input/HUD in `page.tsx`. Lint/type-check/build clean; route serves 200. **The human feel checkpoint is the actual gate and is still PENDING** (needs desktop + 375px phone hand-play — this env has no GPU browser). Start numbers for CB.1 to inherit: G≈900, e≈0.72, ballR=6, pegR=7, ~46 pegs, shots=9. **Next: hand-play the feel checkpoint; if fun → CB.1 deterministic seeded core (`surface/core-breaker.ts`); if not → re-judge the pivot.**
...
- **Last session:** 2026-06-20 — **CB.0 Core Breaker feel prototype** on `claude/brave-hamilton-yds2fo` (`docs/work/2026-06-20-cb0-core-breaker-spike.md`): merged the pivot docs (PR #30), then built the throwaway `/core-breaker-spike` harness — pure `core-breaker-spike.ts` solver (circle-vs-peg restitution, pierce drag, homing steer, bouncy on-rest AoE, settle detection) + a Pixi `page.tsx` (aim-fire pointer input, gravity-projected aim guide, field/ball/burst FX, HUD, ball buttons, live knobs). 240 Hz sub-stepping for anti-tunneling. Code complete + machine-verified (lint/type-check/build/route-200); **human feel checkpoint pending** — that's the pivot gate. Next: hand-play it, then CB.1 (or re-judge).
