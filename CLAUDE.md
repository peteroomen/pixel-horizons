...
- **Current phase:** Phase 2 in progress. Phase 1 complete; 2.1 (combat engine) and 2.2 (combat playable) complete. Next: Slice 2.3 (malfunctions — module targeting, card flipping, play-to-repair, hull innates, Parasite enemy).
- **Last session:** 2026-06-11 — Slice 2.2: combat playable in the browser — `src/game/main.ts` is now the React↔game boundary (`initGame`/`GameHandle`, seed-in-URL resolution, quiet command guards), `combat-view.ts` projects CombatState into a render-ready `CombatView` (rules text derived from effect data), `space-renderer.ts` Pixi battle viewport (placeholder ships, hit flash, shield ring), DOM `CombatHand`/`HUD`, victory/defeat overlays (Fight Again continues the run via new `applyCombatResult`; New Run restarts the seed). Full fight vs Lamprey verified by hand on desktop + mobile viewport. 111 tests green. See `docs/work/2026-06-11-combat-playable.md` and the Slice 2.3 handoff `docs/work/2026-06-11-handoff-slice-2.3.md`.
- **All merged to main:** Scaffold, Slice 1.2 (PR #1), Slice 1.3 (PR #2), Slice 2.1 (PR #3).
- **Open PRs:** Slice 2.2 — `feature/combat-playable`.
- **Deferred:** Hull innate abilities + `repair-all-modules` stub (2.3); travel progress displayed but unconsumed until lanes (2.4); `SaveStore` page wiring (no node boundaries until 2.4/4.5); retain-with-choice API; whether base intent telegraphs become always-on (2.5 — currently hidden unless Deep Scan); Mk II deck generation + tier tracking in RunState (Workbench, 4.2); physical-phone check of production URL; `motion` installed but unused until the card-feel slice (6.6).
