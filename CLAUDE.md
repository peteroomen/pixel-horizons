...
- **Current phase:** Phase 3 in progress; 3.1 complete (pending PR merge). Next: **Slice 3.2 (mining + drop pod)** — deposits, backpack, deposit-at-pod, pod timer + auto-launch, miss-the-window consequence. See `docs/work/2026-06-11-handoff-slice-3.2.md`. Human hand-play still recommended for both the 2.5 fun checkpoint and 3.1 platformer feel.
- **Last session:** 2026-06-11 — Slice 3.1: platformer core (implemented by a Sonnet agent from a fully-specced plan, then review-fixed). `src/game/surface/` stood up React/Pixi-free: `tilemap.ts` (ASCII levels → flat plain-JSON grid, `version` bump on `breakTile` drives tile redraw, OOB = solid until 3.4), `physics.ts` (axis-separated AABB `moveBody`; caller applies gravity), `clone.ts` (run/jump/melee with coyote time, jump buffering, jump-cut — implemented, not stubbed; rising-edge detection in-sim so input scripts are pure data), `surface.ts` orchestrator; all tunables in `data/surface.ts`, hand-made `ROCKY_TEST_LEVEL` in `data/levels.ts` (breakable rocks are the melee target until enemies in 3.4; yields nothing until 3.2). First real 60fps loop: fixed-timestep accumulator on the Pixi ticker in `main.ts` behind a `?mode=surface` dev knob (ADR 004); `surface-renderer.ts` syncs per rAF with a clamped integer camera; keyboard in `main.ts`, `TouchControls.tsx` DOM overlay → `handle.surfaceInput()`; React gets one `onModeChange`, nothing per-frame. **Review caught a real physics bug the agent's tests missed:** `floor((edge-1)/TILE)` tile scans miss sub-pixel penetrations — a resting clone (0.42px gravity nudge/frame) sank through every floor; fixed via `maxEdgeTileIndex = ceil(edge/TILE)-1` + resting-contact regression tests. 233 tests green; browser-verified desktop + 375px incl. combat regression. See `docs/work/2026-06-11-platformer-core.md`.
- **All merged to main:** Scaffold, Slice 1.2 (PR #1), Slice 1.3 (PR #2), Slice 2.1 (PR #3), Slice 2.2 (PR #4), Slice 2.3 (PR #5), Slice 2.4 (PR #6), Slice 2.5 (PR #7).
- **Open PRs:** Slice 3.1 — `feature/platformer-core`.
- **Deferred:** human hand-play of the 2.5 fun checkpoint (bot verdict positive; all knobs in `data/`) and of 3.1 platformer feel (jump arc, coyote/buffer windows, multi-touch on a real phone — knobs in `data/surface.ts`); mining yields from broken rocks + backpack + pod (3.2 — `breakTile` is the hook point); render interpolation + acceleration curves (only if the feel pass demands); pit-death/OOB semantics (3.4 — OOB is solid for now); `?mode=` knob replaced by sector-map mode entry (4.1); anchor escape valve (re-judge with human play); more Infestation variants / on-draw kinds when an enemy needs them; Kinetic Shred (strip-armor) unreachable in play until Mk II decks (4.2); lane modifiers, danger-weighted encounter counts, arrival-on-a-map-node (4.1 — `createLane` is the placeholder generator); `SaveStore` page wiring + abandon-mid-lane resume (4.5); retain-with-choice API; planetside item offline-while-malfunctioning (Phase 3); Mk II deck generation + tier tracking in RunState (Workbench, 4.2); hull select UI (Phase 5); card-flip animation (6.6); physical-phone check of production URL; `motion` installed but unused until the card-feel slice (6.6).
