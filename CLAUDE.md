...
- **Current phase:** Phase 4/5/6. **Slice 4.7 Powers & status effects** (GDD §5.10, ADR 008) is on **PR (`feature/powers-status`)**, plan `docs/work/2026-06-15-powers-status.md` — STS-style visible/stacking statuses + tooltips, buff cards now Exhaust, Tracer Lock now a targeted Mark. Both prior PRs are **merged**: #13 THE ARSENAL and #14 WORLD ART DIRECTION (combat pixel-art renderer). Next candidate: **3.4** (clone death + surface enemies) or **4.8** (Station UX & workbench access); **4.9** Deckbuilding acquisition audit also queued.
...
- **All merged to main:** Scaffold, Slices 1.2–1.3 (PRs #1–2), 2.1–2.5 (PRs #3–7), 3.1 (PR #8), FOUNDRY Combat UI (PR #9), 3.2 (PR #10), THE RUN mega-slice 3.3+4.1+4.5-lite (PR #11), THE FORGE mega-slice 4.2+4.3+5.2 (PR #12), THE ARSENAL mega-slice 4.6+4.4+organs (PR #13), WORLD ART DIRECTION combat renderer (PR #14).
- **Open PRs:** `feature/powers-status` — Slice 4.7 (opening this session).
- **Playtest follow-ups (PR #13, in the roadmap):** #2 STS-style Powers/status system + Exhaust-on-buff + targeted-debuff skills + keyword tooltips → **4.7 shipped** (this session); #3 full module-management UX + #7 workbench access from stations → **4.8**; "too easy" combat + first-try boss kill → 5.5 / 5.2 boss tuning (re-tune the now-Exhaust buff economy then). Phone-verify still pending for the buy-block label and the engineer-repair tap (the touch bug never reproduced on desktop).
