...
- **Current phase:** Phase 2 complete (pending PR merge; human play of the fun checkpoint recommended). Next: **Slice 3.1 (platformer core)** — a mode switch; `src/game/surface/` is untouched scaffolding. See `docs/work/2026-06-11-handoff-slice-3.1.md`.
- **Last session:** 2026-06-11 — Slice 2.5: enemy roster. All five GDD §5.7 archetypes live. Carapace (Bulwark, 30 HP): `EnemyDef.armor` pool absorbs non-piercing damage before HP, regens +2/enemy phase capped at 5 (tuned down from full regen after a sim probe showed Scout/Freighter literally couldn't win); piercing bypasses; `strip-armor` implemented (zeroes the pool). One `dealDamageToEnemy` funnel for cards + damage innates. Sporecaster (Scrambler, 20 HP): new `inject` intent kind inserts Spore Clusters at random draw-pile positions on the combat stream (not a hit — defenses don't block it). Infestations: `CombatCard.moduleIndex` is now `number | null` (null = injected, no module — type-checked, no sentinel), `CardDef.unplayable` (playCard throws; `isCardPlayable` for guards) + `onDraw` effects (own tiny union, deliberately draw-free + RNG-free because `drawCards` is re-entrant); Spore Cluster = unplayable, on-draw lose 1 shield layer; vanish at encounter end for free. Intent telegraphs always-on as `IntentView { kind, name, detail }` — kind+name free (StS-style), numbers behind Deep Scan (user-confirmed design call). HUD: ⛨ ARMOR line + intent line; spore-green unplayable cards, no AP badge, still discardable via Slipstream. Fun checkpoint (bot-level): tense Scout arc (escaped a Carapace by arrival, lost an Anchormaw kill-race by 2 HP), Gunship cruised 13 lanes; scrap stayed 0 all run → **Pay Toll is Freighter-only until 4.3 adds scrap drops** (intentional); no anchor escape valve built. 184 tests green; desktop + 375px verified. See `docs/work/2026-06-11-enemy-roster.md`.
- **All merged to main:** Scaffold, Slice 1.2 (PR #1), Slice 1.3 (PR #2), Slice 2.1 (PR #3), Slice 2.2 (PR #4), Slice 2.3 (PR #5), Slice 2.4 (PR #6).
- **Open PRs:** Slice 2.5 — `feature/enemy-roster`.
- **Deferred:** human hand-play of the 2.5 fun checkpoint (bot verdict positive; all knobs in `data/`); anchor escape valve (re-judge with human play); more Infestation variants / on-draw kinds when an enemy needs them; Kinetic Shred (strip-armor) unreachable in play until Mk II decks (4.2); lane modifiers, danger-weighted encounter counts, arrival-on-a-map-node (4.1 — `createLane` is the placeholder generator); `SaveStore` page wiring + abandon-mid-lane resume (4.5); retain-with-choice API; planetside item offline-while-malfunctioning (Phase 3); Mk II deck generation + tier tracking in RunState (Workbench, 4.2); hull select UI (Phase 5); card-flip animation (6.6); physical-phone check of production URL; `motion` installed but unused until the card-feel slice (6.6).
