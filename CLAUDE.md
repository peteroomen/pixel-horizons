...
- **Current phase:** Phase 2 in progress. Phase 1 complete; 2.1 (combat engine), 2.2 (combat playable), and 2.3 (malfunctions) complete. Next: Slice 2.4 (hyperspace run — lane distance in turns, travel-progress consumption, encounter triggers, escape-by-arrival, malfunctions persisting within a lane, Anchormaw).
- **Last session:** 2026-06-11 — Slice 2.3: malfunctions + hull innates + Parasite. CombatState piles are now `CombatCard { cardId, moduleIndex }` instances (duplicate modules are distinct malfunction targets); flipping is _derived_ — a card presents as its Malfunction form iff its module index is in `CombatState.malfunctioning`. Module hits funnel through `resolveIncomingHit` (a shield layer that absorbs the hit absorbs the malfunction, GDD §5.2); play-to-repair costs `MALFUNCTION_REPAIR_AP` (1); `repair-all-modules` implemented. Hull innates are interpreted data (`InnateAbility.uses` + `InnateEffect` union) via `activateInnate`/`canUseInnate` (named activate- because bare `useX()` calls trip react-hooks lint). Parasite (hunts the highest-value operational module) is fight 2 via roster cycling in `main.ts`; `?hull=` dev knob exercises the other innates. Verified by hand on desktop + 375px. 142 tests green. See `docs/work/2026-06-11-malfunctions.md` and the Slice 2.4 handoff `docs/work/2026-06-11-handoff-slice-2.4.md`.
- **All merged to main:** Scaffold, Slice 1.2 (PR #1), Slice 1.3 (PR #2), Slice 2.1 (PR #3), Slice 2.2 (PR #4).
- **Open PRs:** Slice 2.3 — `feature/malfunctions`.
- **Deferred:** Malfunction persistence across encounters within a lane, travel-progress consumption, and lane-driven encounter selection (2.4 — `main.ts` cycles `ENEMY_DEFS` as a placeholder); `SaveStore` page wiring (no node boundaries until 2.4/4.5); retain-with-choice API; whether base intent telegraphs become always-on (2.5 — hidden unless Deep Scan; module-hunt intents telegraph "→ BEST/RANDOM MODULE" when revealed); `strip-armor` throws until Carapace (2.5); planetside item offline-while-malfunctioning (Phase 3); Mk II deck generation + tier tracking in RunState (Workbench, 4.2); hull select UI (Phase 5); card-flip animation (6.6); physical-phone check of production URL; `motion` installed but unused until the card-feel slice (6.6).
