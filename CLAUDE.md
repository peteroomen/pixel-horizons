...
- **Current phase:** Phase 4/5/6. **Slice 4.7 Powers & status effects** (GDD §5.10, ADR 008) is **merged** (PR #15) — STS-style visible/stacking statuses + tooltips, buff cards now Exhaust, Tracer Lock now a targeted Mark; includes the 375px tooltip-portal/single-open fix. Next candidate: **4.8** (Station UX & workbench access — handoff at `docs/work/2026-06-15-handoff-slice-4.8.md`, reuses the 4.7 `InfoChip` tooltip), with **3.4** (clone death + surface enemies) and **4.9** (deckbuilding acquisition audit) also queued.
