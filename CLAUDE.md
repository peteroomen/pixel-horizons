...
- **Current phase:** Phase 3 in progress; 3.1 merged (PR #8), 3.2 complete (pending PR merge). Next: **Slice 3.3 (module item projection)** — items derived from installed ship modules (double jump, phase dash, scanner, shield bubble…), reactor item cap. See `docs/work/2026-06-12-handoff-slice-3.3.md`. Human hand-play still recommended: 2.5 fun checkpoint, 3.1 platformer feel, and now the 3.2 pod-window pressure at the real 5:00.
- **Last session:** 2026-06-12 — Slice 3.2: mining + drop pod. Deposit tiles ('b' biomineral, 's' scrap cache — solid until broken) yield into a capacity-capped backpack on `CloneState`; `breakTile` now returns the broken tile type and `updateClone` returns `{ brokenTiles }` — `surface.ts` owns the economy (clone.ts knows nothing about yields/capacity, per the 3.3 module-projection warning). `surface/pod.ts`: pod entity (never tiles) from a 'D' level marker, sim-time launch window (`?pod=` seconds dev knob; default 5:00), walk-up auto-deposit on AABB overlap, deposit-before-tick ordering makes "aboard with a full backpack" impossible. Outcomes: 'aboard' (on pod at expiry, everything banked) / 'stranded' (backpack → `lostBackpack`, deposits safe, sim freezes). React: `onSurfaceUpdate` emits a `SurfaceView` only when it differs from the last emission (≈1/sec timer tick + mining/deposit moments); `SurfaceHUD` + POD LAUNCHED overlay with Drop Again (= surface-mode `restartRun`). RunState integration deferred to 4.1 — the run ends at the result screen. 272 tests green; browser-verified desktop + 375px incl. combat regression (aboard overlay variant verified by sim test only — preview-tab rAF throttling). **Session quirk:** implemented in a dedicated worktree (`../pixel-horizons-slice-3.2`) after a parallel foundry-UI session took over the main tree mid-slice; stash@{0} in the main tree holds that session's WIP mixed with superseded copies of this slice's edits. See `docs/work/slice-3.2-plan.md`.
- **All merged to main:** Scaffold, Slice 1.2 (PR #1), Slice 1.3 (PR #2), Slice 2.1 (PR #3), Slice 2.2 (PR #4), Slice 2.3 (PR #5), Slice 2.4 (PR #6), Slice 2.5 (PR #7), Slice 3.1 (PR #8).
- **Open PRs:** Slice 3.2 — `feature/mining-drop-pod`.
- **Deferred:** human hand-play of the 2.5 fun checkpoint (bot verdict positive; all knobs in `data/`) and of 3.1 platformer feel (jump arc, coyote/buffer windows, multi-touch on a real phone — knobs in `data/surface.ts`); module-driven backpack capacity + Engine-quality pod-window extension (3.3 — constants in `data/surface.ts` are the hook points); banked-resources → `RunState.resources` + early/manual pod launch + pod-defense events (4.1+); render interpolation + acceleration curves (only if the feel pass demands); pit-death/OOB semantics (3.4 — OOB is solid for now); `?mode=` knob replaced by sector-map mode entry (4.1); anchor escape valve (re-judge with human play); more Infestation variants / on-draw kinds when an enemy needs them; Kinetic Shred (strip-armor) unreachable in play until Mk II decks (4.2); lane modifiers, danger-weighted encounter counts, arrival-on-a-map-node (4.1 — `createLane` is the placeholder generator); `SaveStore` page wiring + abandon-mid-lane resume (4.5); retain-with-choice API; planetside item offline-while-malfunctioning (Phase 3); Mk II deck generation + tier tracking in RunState (Workbench, 4.2); hull select UI (Phase 5); card-flip animation (6.6); physical-phone check of production URL; `motion` installed but unused until the card-feel slice (6.6).
