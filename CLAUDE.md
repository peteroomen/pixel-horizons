...
- **Current phase:** Phase 4 in progress. THE RUN mega-slice (3.3 module item projection + 4.1 sector map/run loop + 4.5-lite save/resume) is on **PR #11** (`feature/the-run`), including the 2026-06-13 playtest fixes. Next candidate: mega-slice 2 "THE FORGE" (4.2 workbench + 4.3 shops/economy + 5.2 sector boss) — plan drafted at `docs/work/mega-slice-2-plan.md`, **on hold pending Peter's go**.
- **Last session:** 2026-06-13 — playtest fixes on `feature/the-run` (see `docs/work/2026-06-13-playtest-fixes.md`): (1) **early pod return** — `launchPod`/`canLaunchPod` in `surface/surface.ts`, `SurfaceView.canLaunch`, top-center LAUNCH POD button when the clone stands on the pod (deposits backpack, outcome 'aboard'); (2) **abandon planet** — new outcome `'abandoned'` + `abandonSurface` (= stranded semantics: backpack lost, deposits safe), always-visible ABANDON button with two-tap confirm (2.5 s disarm); (3) **combat only in lanes** — removed the `'combat'` node type, `NODE_COMBAT_LANE`, and the forced-encounter branch in `main.ts`; nodes are realspace destinations (GDD §6.2/§7.1–7.3 updated — forced fights become lane properties: infested/elite lanes), node weights now `{ planet: 4, cache: 2 }`. 325 tests green; browser-verified desktop + 375 px (map without combat nodes; lane encounter → escape-by-arrival → cache banked with no node fight; launch + abandon flows). Follow-up: `?modules=` / `?reactor=` dev knobs (see Dev Knobs below) so every item projection is testable before the Workbench exists.
