...
- **CB VISUAL PASS (merged via PR #38)** (`docs/work/2026-06-22-cb-visual-pass.md`) — _the renderer/HUD details below were superseded in the same PR by the full-screen portrait + React HUD work; see the ✅ bullet above for the final shape:_ the Core Breaker renderer is now pixel-art. New `src/renderer/core-breaker/` modules (`sprite-toolkit`, `peg-sprites`, `ball-sprites`, `background`, `layout`, `hud`) port the design prototype (`docs/design/mining-run-v2.dc.html`) — 6 silhouette peg sprites (×damage stages), 5 ball sprites, crust+ceiling-scar background, FOUNDRY header + roster tray (incl. reprint/return). `core-breaker-renderer.ts` is now a thin orchestrator with two nested contain-fits (portrait "column" → host viewport; cfg sim space → playfield band between header/tray) so the **main game shows mining as a centered column** in the landscape stage with no app changes. `enterMining()` now uses `portraitConfig()` + passes `viewport`/`biome`. Sprites baked via the prototype's 2D-canvas toolkit → nearest PixiJS textures (no sim/test churn). **562 tests green**, type-check/lint/build clean. Standalone `/core-breaker` fully browser-verified; the in-game orbit→mining transition handoff needs a focused-browser/device check (rAF throttling in the headless preview kept the 3.6s `pod-deploy` transition from completing). **Next: CB.5 ball glyph grammar + dual-face `<ModuleCard>`, CB.6 Bloom soft-fail, CB.7 cleanup.**
- **Current phase:** Phase 6 / Core Breaker pivot. **PORTRAIT REDESIGN (merged via PR #38)** (`docs/work/2026-06-22-mining-portrait-redesign.md`): pod moves to the top of the screen (podY≈30), fires balls downward; mineral drops auto-magnet back up (700 px/s², no player input); `ShotEnd.caught` and `StepEvents.caught` removed (ball expires at floor = fellOut). `defaultConfig()` kept landscape for main game; new `portraitConfig()` (360×640) used by standalone `/core-breaker` route. `field-gen.ts` detects orientation via `cfg.height > cfg.width` and branches on LS*\*/PT*\* constants. Two bugs caught in browser testing and fixed: (1) map-screen ghost of mining background (root: defaultConfig was portrait, main game enterMining drew tall rect); (2) portrait orientation not applying (root: same cause). Both resolved by the defaultConfig/portraitConfig split. 562 tests green, type-check/lint/build clean. (Merged via PR #38; superseded by full-screen portrait + React HUD — see the ✅ bullet above.)
- **✅ SPACE ALWAYS ON built (branch `claude/game-map-space-ui-sfw624`, 2026-06-22, PR pending).** Persistent portrait canvas (360×720) with animated 3-layer parallax starfield as the permanent bottom layer of `app.stage`. All game phases now run in portrait orientation — `stageView` is computed once at boot via `coreBreakerViewport()` and never changes. React overlays (map, shop, event, title) become solid Foundry metal panels (`bg-fd-void` + `border-fd-steel/40`) floating over transparent containers so space is always visible around them. Ships in combat are offset — player lower-left (38%/71%), enemy upper-right (62%/29%) — diagonal dogfight angle. **562 tests green**, type-check clean, lint clean. Browser verification needed: starfield behind title panel, transparent map, portrait combat, orbit in portrait. **Next: browser verify → merge PR → CB.5 ball glyph grammar.**
- **✅ CORE BREAKER IS LIVE — MERGED to main (PR #38, commit `55fff53`, 2026-06-22).** Portrait redesign + pixel-art visual pass + full-screen portrait + unified single-session bootstrap (`startCoreBreaker`) + **React-DOM HUD** (ADR 001 / 012) all landed in one squash. It's the live surface loop now, reachable two ways from one codepath: the in-game flow (orbit → DROP → `enterMining`, which flips the stage to portrait) and the `/core-breaker` route; `?mode=mining` jumps straight in. **562 tests green.** Full handover: `docs/work/2026-06-22-handoff-core-breaker.md`. **Next: CB.5 ball glyph grammar + dual-face `<ModuleCard>`, CB.6 Bloom soft-fail, CB.7 platformer cleanup.**
- **Last session:** 2026-06-22 — **SPACE ALWAYS ON** — persistent portrait canvas + animated starfield + solid Foundry metal overlays. Branch `claude/game-map-space-ui-sfw624`, PR pending. 562 tests green.
- **Prior session:** 2026-06-22 — **PORTRAIT REDESIGN + MINING V2** — both folded into PR #38 and merged to main. (PRs #35/#36 were intermediate; their work lives in PR #38.) 562 tests green.
...
| `?mode=`                  | `?mode=surface`                                     | Skips the run loop and drops straight onto the test planet with the resolved loadout; no save interaction. `?mode=orbit` drops into the orbit screen (generated planet + DROP → mining); `?mode=mining` drops **straight into the portrait Core Breaker mining run** (the live surface loop) with the resolved loadout — for feel-tuning without the orbit/drop dance |
...
| `?pod=`                   | `?pod=45`                                           | Base pod launch window in seconds (engine bonuses still apply on top)                                                                                                                                                                                                                                                                                                 |
| `?mode=`                  | `?mode=surface`                                     | Skips the run loop and drops straight onto the test planet with the resolved loadout; no save interaction. `?mode=orbit` drops into the orbit screen (generated planet + DROP → mining); `?mode=mining` drops **straight into the portrait Core Breaker mining run** (the live surface loop) with the resolved loadout — for feel-tuning without the orbit/drop dance |
| _(route `/core-breaker`)_ | `/core-breaker?seed=&modules=&reactor=&difficulty=` | Standalone Core Breaker — one of two entry paths to the **same** run as in-game (both call `startCoreBreaker`); the route owns its own Pixi app + portrait scaling. Deterministic from `?seed=`. Identical experience to `?mode=mining`; only the door differs.                                                                                                       |
