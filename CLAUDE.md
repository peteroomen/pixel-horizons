...
- **Current phase:** Phase 2 in progress. Phase 1 complete; 2.1–2.4 complete. Next: Slice 2.5 (enemy roster — Carapace + strip-armor, Sporecaster + Infestation cards, intents telegraphed in UI, **fun checkpoint: tune before proceeding**).
- **Last session:** 2026-06-11 — Slice 2.4: hyperspace runs. New `sim/travel.ts`: `LaneState` (plain JSON, owned by `main.ts` _next to_ RunState — a lane is between nodes, ADR 003 saves at node boundaries), `createLane` rolls distance/encounters/enemies on the map-gen stream, `advanceLane` skips encounter points overshot mid-combat (= "faster traversal, fewer encounters"; unused travel carries). Combat takes a frozen `LaneContext`: malfunction carry-in, +1 travel per survived turn, engine cards can end a fight mid-turn via the new `'escaped'` outcome (no victory rewards; arrival also ends the lane and clears malfunctions). Anchormaw is an `EnemyDef.anchor` _trait_ (not an intent): travel halted while it lives, latch always visible in the HUD, `payToll` escapes as a negative `scrapGained` delta (`scrapAtStart` frozen in CombatState for affordability). `main.ts` chains lanes (`continueTravel` replaces `nextFight`; `?enemy=` dev knob forces encounters). HUD: `TRAVEL n/D`, total `SCRAP`, ⚓ TRAVEL HALTED, Pay Toll; overlays: VICTORY/ARRIVED/TOLL PAID/DEFEAT. Verified desktop + 375px. 165 tests green. See `docs/work/2026-06-11-hyperspace-run.md` and the Slice 2.5 handoff `docs/work/2026-06-11-handoff-slice-2.5.md`.
- **All merged to main:** Scaffold, Slice 1.2 (PR #1), Slice 1.3 (PR #2), Slice 2.1 (PR #3), Slice 2.2 (PR #4), Slice 2.3 (PR #5).
- **Open PRs:** Slice 2.4 — `feature/hyperspace-run`.
- **Deferred:** Infestation cards need a `CombatCard.moduleIndex` story for module-less injected cards (2.5); `strip-armor` throws until Carapace (2.5); whether base intent telegraphs become always-on (2.5 — hidden unless Deep Scan); wasted-engine-cards-vs-anchor escape valve (judge at the 2.5 fun checkpoint); lane modifiers, danger-weighted encounter counts, arrival-on-a-map-node (4.1 — `createLane` is the placeholder generator); `SaveStore` page wiring + abandon-mid-lane resume (4.5); retain-with-choice API; planetside item offline-while-malfunctioning (Phase 3); Mk II deck generation + tier tracking in RunState (Workbench, 4.2); hull select UI (Phase 5); card-flip animation (6.6); physical-phone check of production URL; `motion` installed but unused until the card-feel slice (6.6).
