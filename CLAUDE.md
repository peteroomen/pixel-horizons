...
- **Last session:** 2026-06-18 — **SURFACE RECOLOR 6.1 slice 2** (`docs/work/2026-06-18-6.1-slice2-surface-recolor.md`): added `surfaceRampFor(descriptor)` (`renderer/palette.ts`, +4 tests); `P: Ramp` param threaded through every rock-bearing factory in `surface-sprites.ts` (tiles, deposit rock hosts, FX, **backdrop cave-mouth**); `surface-renderer.ts` `createSurfaceRenderer(app, terrainRamp)`; `terrainRamp` on `SurfaceModeOptions` (`surface-mode.ts`); `currentPlanet` wiring in `main.ts`. **Scope grew mid-session** (user-approved) from tiles-only to include the cave-mouth backdrop rock, because tiles-only was barely visible: `ROCKY_TEST_LEVEL` is ~90% bedrock (fixed) and the backdrop was the dominant fixed-rust visual. Verification gotcha pinned: the recolor _looked_ broken at first — pixel-reading the generated tile **canvas** (not the screen) proved it correct per seed; the muted on-screen result was the level/backdrop composition, not the pipeline.
