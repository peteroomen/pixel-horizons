...
- **Last session:** 2026-06-12 — FOUNDRY Combat UI design system. Added `fd-` prefixed design tokens (5-color semantic palette + neutrals + card-state fills), chamfer utilities (`@utility chamfer*`), Silkscreen + VT323 fonts via next/font. Built primitives (`Plate`, `FoundryButton`, `StatBar`) in `src/components/foundry/`. Built `CombatCard` (5 states: normal, malfunction/amber, infested/green, discard/cyan + unaffordable opacity modifier), `PlayerPlate`, `EnemyPlate` (with fused intent chip), `MetaStrip`, `ButtonBar`. Rewrote `HUD.tsx` as thin orchestrator. Restyled `page.tsx` (bg-fd-void, FoundryButton outcome overlay). Added `CardView.infested?: boolean` (additive, unset). Killed all hardcoded hex colors and `retro` class from combat path. 272 tests green. See `docs/work/foundry-ui-plan.md`.
...
- **Open PRs:** FOUNDRY Combat UI — `feature/foundry-ui`.
